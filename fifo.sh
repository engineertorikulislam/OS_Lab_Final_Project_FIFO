...
temp[$frame]=""                                              # Temp Innitialize(Rearrange Frame)
...
do
    ...
    for (( j = 0; j < frame; j++)); 
    do
        if [ "${value[$i]}" == "${temp[$j]}" ]; then  # Push pages in the queue one at a time until the queue reaches its maximum capacity or all page requests are fulfilled.
    ...
    if [[ $page_fault -le $frame ]] && [[ $count -eq 0 ]]; then
        ...
        let $((temp[i] = value[i]))                                # IPush pages in the queue one at a time until the queue reaches its maximum capacity or all page requests are fulfilled.
